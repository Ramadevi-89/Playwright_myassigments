...
test("Create opportunity", async ({ page }) => {
  ...
  //await page.getByRole("combobox", {name: "CloseDate"}).fill("25/02/2026");
  await page.locator('input[type="text"][name="CloseDate"]').fill('2/6/2026');
  ...
  await page.locator('input[type="text"][name="Name"]').fill('Ramadevi Anand');

  await page.locator("//span[@class='slds-truncate' and text()='--None--']").first().click()
  await page.locator("//lightning-base-combobox-item//span[contains(@title,'Qualification')]").click()
